        ...
        <div className="section-container relative">
          <FadeIn>
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-dark mb-6">
              ...
              <EditableText as="span" blockKey={kt('hero.eyebrow')} fallback="결제단말기"
                value={pickTextOrUndef(blocks, kt('hero.eyebrow'))} pagePath={PAGE_TERM}
          ...
          <FadeIn delay={100}>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white tracking-tight leading-[1.12] max-w-3xl break-keep">
              <EditableText as="span" blockKey={kt('hero.title.line1')} fallback="매출은 올리고"
                value={pickTextOrUndef(blocks, kt('hero.title.line1'))} pagePath={PAGE_TERM} /><br />
              <span className="text-gradient">
                <EditableText as="span" blockKey={kt('hero.title.highlight')} fallback="수수료는 낮추는"
                  value={pickTextOrUndef(blocks, kt('hero.title.highlight'))} pagePath={PAGE_TERM} />
              </span><br />
              <EditableText as="span" blockKey={kt('hero.title.line3')} fallback="카드단말기"
                value={pickTextOrUndef(blocks, kt('hero.title.line3'))} pagePath={PAGE_TERM} />
            </h1>
          </FadeIn>

          <FadeIn delay={200}>
            <EditableText as="p" blockKey={kt('hero.description')}
              fallback="유선·무선·키인 단말기부터 POS까지. 업종과 매출에 맞는 최적의 단말기를 비교해드려요."
              value={pickTextOrUndef(blocks, kt('hero.description'))} pagePath={PAGE_TERM}
              ...
              <form onSubmit={handleConsultSubmit}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['무선단말기', '유선단말기', 'POS', '키인단말기'].map((item) => (
                      <label key={item} className={`inline-flex items-center px-4 py-2 rounded-full border text-sm cursor-pointer transition-all ${formProducts.includes(item) ? 'bg-violet-500/20 border-violet-400/40 text-violet-300' : 'bg-white/[0.06] border-white/[0.1] text-gray-300'}`}>
                ...
                <div className="space-y-2 pt-2">
                  <label className="flex items-start gap-2 text-xs text-gray-400 cursor-pointer">
                    ...
                    <span>[필수] 개인정보 수집·이용 및 제3자 제공에 동의합니다. <Link href="/privacy" className="text-violet-400 underline">자세히 보기</Link></span>
                  </label>
                  <label className="flex items-start gap-2 text-xs text-gray-400 cursor-pointer">
                ...
                <button type="submit" disabled={submitting}
                  className="w-full mt-2 inline-flex items-center justify-center rounded-xl bg-violet-500 px-6 py-4 text-base font-semibold text-white hover:bg-violet-600 hover:shadow-[0_0_40px_rgba(139,92,246,0.3)] active:scale-[0.98] transition-all duration-400 ease-toss disabled:opacity-50 disabled:cursor-not-allowed">
