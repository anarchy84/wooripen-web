...
const PAGE_RENT = '/rental'
const kr = (s: string) => `rental.${s}`
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-gray-950 pt-32 pb-20 md:pt-40 md:pb-28">
        ...
        <EditableImage
          blockKey={kr('hero.background')}
          fallback={{ url: '', alt: '매장 렌탈 배경' }}
          value={pickImageOrUndef(blocks, kr('hero.background'))}
          pagePath={PAGE_RENT}
        ...
        <div className="section-container relative">
          <FadeIn>
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-dark mb-6">
              ...
              <EditableText as="span" blockKey={kr('hero.eyebrow')} fallback="매장 렌탈"
                value={pickTextOrUndef(blocks, kr('hero.eyebrow'))} pagePath={PAGE_RENT}
          ...
          <FadeIn delay={100}>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white tracking-tight leading-[1.12] max-w-3xl break-keep">
              <EditableText as="span" blockKey={kr('hero.title.line1')} fallback="초기 비용 0원,"
                value={pickTextOrUndef(blocks, kr('hero.title.line1'))} pagePath={PAGE_RENT} /><br />
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 via-teal-400 to-cyan-400">
                <EditableText as="span" blockKey={kr('hero.title.highlight')} fallback="매장에 필요한 건"
                  value={pickTextOrUndef(blocks, kr('hero.title.highlight'))} pagePath={PAGE_RENT} />
              </span><br />
              <EditableText as="span" blockKey={kr('hero.title.line3')} fallback="전부 렌탈로"
                value={pickTextOrUndef(blocks, kr('hero.title.line3'))} pagePath={PAGE_RENT} />
            </h1>
          </FadeIn>

          <FadeIn delay={200}>
            <EditableText as="p" blockKey={kr('hero.description')}
              fallback="정수기·공기청정기·복합기·제빙기까지. 매장 운영에 필요한 장비를 부담 없이 렌탈하세요."
              value={pickTextOrUndef(blocks, kr('hero.description'))} pagePath={PAGE_RENT}
